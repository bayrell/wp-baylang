"use strict;"
/*!
 *  BayLang Technology
 *
 *  (c) Copyright 2016-2024 "Ildar Bikmamatov" <support@bayrell.org>
 *
 *  Licensed under the Apache License, Version 2.0 (the "License");
 *  you may not use this file except in compliance with the License.
 *  You may obtain a copy of the License at
 *
 *      https://www.apache.org/licenses/LICENSE-2.0
 *
 *  Unless required by applicable law or agreed to in writing, software
 *  distributed under the License is distributed on an "AS IS" BASIS,
 *  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *  See the License for the specific language governing permissions and
 *  limitations under the License.
 */
if (typeof Runtime == 'undefined') Runtime = {};
if (typeof Runtime.WordPress == 'undefined') Runtime.WordPress = {};
if (typeof Runtime.WordPress.Api == 'undefined') Runtime.WordPress.Api = {};
Runtime.WordPress.Api.TermApi = class extends Runtime.Widget.Crud.CrudApi
{
	/**
	 * Returns api name
	 */
	static getApiName(){ return "runtime.wordpress.term"; }
	
	
	/**
	 * Returns table name
	 */
	static getTableName(){ return "terms"; }
	
	
	/**
	 * Returns max limit
	 */
	getMaxLimit(){ return -1; }
	
	
	/**
	 * Returns item fields
	 */
	getItemFields(action)
	{
		return Runtime.Vector.create([
			"term_id",
			"name",
			"slug",
			"taxonomy",
			"parent_id",
			"count",
		]);
	}
	
	
	/**
	 * Returns query field
	 */
	getQueryField(field_name)
	{
		if (field_name == "parent_id")
		{
			return new Runtime.ORM.QueryField("term_taxonomy", "parent", field_name);
		}
		if (field_name == "taxonomy" || field_name == "count")
		{
			return new Runtime.ORM.QueryField("term_taxonomy", field_name);
		}
		return new Runtime.ORM.QueryField("terms", field_name);
	}
	
	
	/**
	 * Build search query
	 */
	async buildSearchQuery(q)
	{
		let taxonomy_name = this.post_data.get("taxonomy", "category");
		q.where("term_taxonomy.taxonomy", "=", taxonomy_name);
		q.innerJoin("term_taxonomy", Runtime.Vector.create(["term_taxonomy.term_id = terms.term_id"]));
		q.orderBy("terms.name", "asc");
		/* Set term id */
		if (this.post_data.has("term_id"))
		{
			let term_id = Runtime.rtl.toInt(this.post_data.get("term_id"));
			q.where("terms.ID", "=", term_id);
			q.calcFoundRows(false);
		}
		/* Set slug */
		if (this.post_data.has("slug"))
		{
			let slug = this.post_data.get("slug");
			q.where("terms.slug", "=", slug);
		}
	}
	
	
	/**
	 * Convert item
	 */
	convertItem(fields, item)
	{
		item.set("count", Runtime.rtl.toInt(item.get("count")));
		item.set("parent_id", Runtime.rtl.toInt(item.get("parent_id")));
		item.set("term_id", Runtime.rtl.toInt(item.get("term_id")));
		return super.convertItem(fields, item);
	}
	
	
	/**
	 * Action search
	 */
	async actionSearch()
	{
		await super.actionSearch();
	}
	
	
	/**
	 * Action search one
	 */
	async actionSearchOne()
	{
		await super.actionSearchOne();
	}
	
	
	/* ========= Class init functions ========= */
	_init()
	{
		super._init();
	}
	static getClassName(){ return "Runtime.WordPress.Api.TermApi"; }
	static getMethodsList()
	{
		const Vector = use("Runtime.Vector");
		return new Vector("actionSearch", "actionSearchOne");
	}
	static getMethodInfoByName(field_name)
	{
		const Vector = use("Runtime.Vector");
		if (field_name == "actionSearch") return new Vector(
			new Runtime.Web.Annotations.ApiMethod()
		);
		if (field_name == "actionSearchOne") return new Vector(
			new Runtime.Web.Annotations.ApiMethod()
		);
		return null;
	}
	static getInterfaces(){ return []; }
};
window["Runtime.WordPress.Api.TermApi"] = Runtime.WordPress.Api.TermApi;