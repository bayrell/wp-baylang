...
Runtime.WordPress.Database.Migrations.Core_2025_01 = function()
{
	Runtime.ORM.BaseMigration.apply(this, arguments);
};
Runtime.WordPress.Database.Migrations.Core_2025_01.prototype = Object.create(Runtime.ORM.BaseMigration.prototype);
Runtime.WordPress.Database.Migrations.Core_2025_01.prototype.constructor = Runtime.WordPress.Database.Migrations.Core_2025_01;
Object.assign(Runtime.WordPress.Database.Migrations.Core_2025_01.prototype,
{
	/**
	 * Create forms
	 */
	wp_forms: function()
	{
		return new Runtime.ORM.BaseMigration(Runtime.Map.from({"up":async () =>
		{
			var table_name = this.connection.getTableName("forms");
			this.comment("Create table " + Runtime.rtl.toStr(table_name));
			await this.executeSQL(Runtime.Vector.from(["CREATE TABLE `" + Runtime.rtl.toStr(table_name) + Runtime.rtl.toStr("` ("),"  `id` bigint(20) NOT NULL AUTO_INCREMENT,","  `name` varchar(255) NOT NULL,","  `api_name` varchar(255) NOT NULL,","  `settings` text NOT NULL,","  `email_to` varchar(255) NOT NULL,","  `is_deleted` tinyint(4) NOT NULL DEFAULT 0,","  PRIMARY KEY (`id`)",") ENGINE=InnoDB"]));
		},"down":async () =>
		{
			var table_name = this.connection.getTableName("forms");
	 ...
	 * Create forms
	 */
	wp_forms_data: function()
	{
		return new Runtime.ORM.BaseMigration(Runtime.Map.from({"up":async () =>
		{
			var table_name = this.connection.getTableName("forms_data");
			this.comment("Create table " + Runtime.rtl.toStr(table_name));
	 ...
	 * Create forms_ip
	 */
	wp_forms_ip: function()
	{
		return new Runtime.ORM.BaseMigration(Runtime.Map.from({"up":async () =>
		{
			var table_name = this.connection.getTableName("forms_ip");
			this.comment("Create table " + Runtime.rtl.toStr(table_name));
	...
	wp_gallery: function()
	{
		return new Runtime.ORM.BaseMigration(Runtime.Map.from({"up":async () =>
		{
			var table_name = this.connection.getTableName("gallery");
			this.comment("Create table " + Runtime.rtl.toStr(table_name));
			await this.executeSQL(Runtime.Vector.from(["CREATE TABLE `" + Runtime.rtl.toStr(table_name) + Runtime.rtl.toStr("` ("),"  `id` bigint(20) NOT NULL AUTO_INCREMENT,","  `api_name` varchar(255) NOT NULL,","  `is_deleted` tinyint(4) NOT NULL DEFAULT 0,","  PRIMARY KEY (`id`)",") ENGINE=InnoDB"]));
		},"down":async () =>
		{
			var table_name = this.connection.getTableName("gallery");
			this.executeSQL(Runtime.Vector.from(["DROP TABLES `" + Runtime.rtl.toStr(table_name) + Runtime.rtl.toStr("`")]));
	 ...
	 * Create gallery item
	 */
	wp_gallery_item: function()
	{
		return new Runtime.ORM.BaseMigration(Runtime.Map.from({"up":async () =>
		{
			var table_name = this.connection.getTableName("gallery_item");
			var table_name_gallery = this.connection.getTableName("gallery");
			this.comment("Create table " + Runtime.rtl.toStr(table_name));
			await this.executeSQL(Runtime.Vector.from(["CREATE TABLE `" + Runtime.rtl.toStr(table_name) + Runtime.rtl.toStr("` ("),"  `id` bigint(20) NOT NULL AUTO_INCREMENT,","  `gallery_id` bigint(20) NOT NULL,","  `image_id` bigint(20) NOT NULL,","  `name` varchar(255) NOT NULL,","  `pos` bigint(20) NOT NULL,","  `is_deleted` tinyint(4) NOT NULL DEFAULT 0,","  KEY `pos` (`pos`),","  PRIMARY KEY (`id`)",") ENGINE=InnoDB"]));
			this.comment("Create foreign key " + Runtime.rtl.toStr(table_name) + Runtime.rtl.toStr("_id"));
			await this.executeSQL(Runtime.Vector.from(["ALTER TABLE `" + Runtime.rtl.toStr(table_name) + Runtime.rtl.toStr("`"),"  ADD CONSTRAINT `" + Runtime.rtl.toStr(table_name) + Runtime.rtl.toStr("_id`"),"  FOREIGN KEY (`gallery_id`)","  REFERENCES `" + Runtime.rtl.toStr(table_name_gallery) + Runtime.rtl.toStr("`(`id`)"),"  ON DELETE RESTRICT ON UPDATE CASCADE"]));
		},"down":async () =>
		{
			var table_name = this.connection.getTableName("gallery_item");
			await this.executeSQL(Runtime.Vector.from(["ALTER TABLE `" + Runtime.rtl.toStr(table_name) + Runtime.rtl.toStr("`"),"  DROP FOREIGN KEY `" + Runtime.rtl.toStr(table_name) + Runtime.rtl.toStr("_id`")]));
			this.executeSQL(Runtime.Vector.from(["DROP TABLES `" + Runtime.rtl.toStr(table_name) + Runtime.rtl.toStr("`")]));
		}}));
	},
	/**
	 ...
	 */
	wp_mail_delivery: function()
	{
		return new Runtime.ORM.BaseMigration(Runtime.Map.from({"up":async () =>
		{
			var table_name = this.connection.getTableName("mail_delivery");
			this.comment("Create table " + Runtime.rtl.toStr(table_name));
	 ...
	 */
	wp_mail_settings: function()
	{
		return new Runtime.ORM.BaseMigration(Runtime.Map.from({"up":async () =>
		{
			var table_name = this.connection.getTableName("mail_settings");
			this.comment("Create table " + Runtime.rtl.toStr(table_name));
	...
	_init: function()
	{
		Runtime.ORM.BaseMigration.prototype._init.call(this);
...
Object.assign(Runtime.WordPress.Database.Migrations.Core_2025_01, Runtime.ORM.BaseMigration);
Object.assign(Runtime.WordPress.Database.Migrations.Core_2025_01,
{
...
Runtime.rtl.defClass(Runtime.WordPress.Database.Migrations.Core_2025_01);
window["Runtime.WordPress.Database.Migrations.Core_2025_01"] = Runtime.WordPress.Database.Migrations.Core_2025_01;
if (typeof module != "undefined" && typeof module.exports != "undefined") module.exports = Runtime.WordPress.Database.Migrations.Core_2025_01;