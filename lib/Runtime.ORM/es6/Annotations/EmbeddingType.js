...
Runtime.ORM.Annotations.EmbeddingType = class extends Runtime.ORM.Annotations.BaseType
{
	/**
	...
	async fromDatabase(conn, item)
	{
		if (item.has(this.name))
		{
			...
			let obj = Runtime.rtl.jsonDecode(value);
	...
	padding(value)
	{
		...
		{
			value = value.slice(0, Runtime.rtl.min(count, this.dimension));
			if (count < this.dimension)
			{
				for (let i = 0; i < this.dimension - count; i++)
				{
					value.push(0);
				}
			}
	...
	async toDatabase(conn, item)
	{
		...
		item.set(this.name, Runtime.rtl.jsonEncode(value));
	...
	static getClassName(){ return "Runtime.ORM.Annotations.EmbeddingType"; }
	static getMethodsList(){ return null; }
	static getMethodInfoByName(field_name){ return null; }
	static getInterfaces(){ return []; }
};
window["Runtime.ORM.Annotations.EmbeddingType"] = Runtime.ORM.Annotations.EmbeddingType;