...
Object.assign(Runtime.ORM.MySQL.SQLBuilder.prototype,
{
		...
		else if (q._kind == Runtime.ORM.Query.QUERY_UPDATE)
		{
			...
			if (q._data)
			{
				q._data.each((value, key) =>
				{
					...
					var field_name = this.prepareField(key);
	...
	convertFilterItem: function(item, data, field_index)
	{
		...
		if (value === null)
		{
			if (op == "!=")
			{
				var s = this.prepareField(field_name) + Runtime.rtl.toStr(" is not null");
				return Runtime.Vector.from([s,field_index]);
			}
			else
			{
				var s = this.prepareField(field_name) + Runtime.rtl.toStr(" is null");
				return Runtime.Vector.from([s,field_index]);
			}
		}
		else if (value instanceof Runtime.Collection)
		{
			if (op == "=")
			{
				...
				else
				{
					...
					var s = this.prepareField(field_name) + Runtime.rtl.toStr(" in (") + Runtime.rtl.toStr(Runtime.rs.join(",", res)) + Runtime.rtl.toStr(")");
					return Runtime.Vector.from([s,field_index]);
				}
			}
		}
		else
		{
			...
			var res = this.prepareValue(value, op);
