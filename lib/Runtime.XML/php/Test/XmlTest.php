...
class XmlTest
{
	...
	{
		$s1 = $xml1->toString(new \Runtime\Map(["indent" => false, "output-xml" => true]));
	...
	static function getClassName(){ return "Runtime.XML.Test.XmlTest"; }
	static function getMethodsList()
	{
		return new \Runtime\Vector("testLoadXml", "testChilds", "testIncorrect", "testVersion_1_1", "testCountZero", "testCountOne", "testCountTwo", "testAttributes1", "testAttributes2", "testRemoveItem", "testEmptyItem");
	}
	static function getMethodInfoByName($field_name)
	{
		if ($field_name == "testLoadXml") return new \Runtime\Vector(
			new \Runtime\Unit\Test(new \Runtime\Map())
		);
		if ($field_name == "testChilds") return new \Runtime\Vector(
			new \Runtime\Unit\Test(new \Runtime\Map())
		);
		if ($field_name == "testIncorrect") return new \Runtime\Vector(
			new \Runtime\Unit\Test(new \Runtime\Map())
		);
		if ($field_name == "testVersion_1_1") return new \Runtime\Vector(
			new \Runtime\Unit\Test(new \Runtime\Map())
		);
		if ($field_name == "testCountZero") return new \Runtime\Vector(
			new \Runtime\Unit\Test(new \Runtime\Map())
		);
		if ($field_name == "testCountOne") return new \Runtime\Vector(
			new \Runtime\Unit\Test(new \Runtime\Map())
		);
		if ($field_name == "testCountTwo") return new \Runtime\Vector(
			new \Runtime\Unit\Test(new \Runtime\Map())
		);
		if ($field_name == "testAttributes1") return new \Runtime\Vector(
			new \Runtime\Unit\Test(new \Runtime\Map())
		);
		if ($field_name == "testAttributes2") return new \Runtime\Vector(
			new \Runtime\Unit\Test(new \Runtime\Map())
		);
		if ($field_name == "testRemoveItem") return new \Runtime\Vector(
			new \Runtime\Unit\Test(new \Runtime\Map())
		);
