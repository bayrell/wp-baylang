...
Object.assign(BayLang.Constructor.Backend.Api.ModuleSearchApi.prototype,
{
	...
	{
		var service = new BayLang.Constructor.Backend.Service.ProjectService();
		/* Load project */
		await service.loadItem(Runtime.Map.from({"id":Runtime.rtl.attr(this.post_data, ["foreign_key", "project_id"])}));
		/* Get modules */
		var modules = await service.getModules();
		/* Set result */
