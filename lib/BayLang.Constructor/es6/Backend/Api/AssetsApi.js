...
Object.assign(BayLang.Constructor.Backend.Api.AssetsApi.prototype,
{
	...
	uploadFile: async function()
	{
		...
		var folder_path = Runtime.fs.join(Runtime.Vector.from([service.assets_path,folder_name]));
	...
	removeFile: async function()
	{
		...
		var file_path = Runtime.fs.join(Runtime.Vector.from([service.assets_path,"images",file_name]));
		/* Check file path */
	...
	getFiles: async function()
	{
		...
		await service.loadProject(project_id);
		/* Get assets path */
		var assets_path = BayLang.Constructor.Backend.ApiHook.getAssetsPath(service.project);
		var assets_url_path = BayLang.Constructor.Backend.ApiHook.getAssetsUrlPath(service.project);
		var folder_name = this.post_data.get("path");
