...
class Editor extends \Runtime\Widget\Field
{
	...
	function render()
	{
		...
		for ($i = 0; $i < $this->lines->count(); $i++)
		{
			...
			/* Text */
			$this->_t($__v3, $this->_escape($i + 1));
			
			/* Element 'div' */
			$this->_e($__v2, "div", ["style" => "height: " . \Runtime\rtl::toStr($this->lines->get($i)) . \Runtime\rtl::toStr("px"),"class" => $this->_class_name(["line"])], $__v3);
		}
		
		/* Element 'div' */
		$this->_e($__v1, "div", ["class" => $this->_class_name(["widget_text__wrap"])], $__v2);
