...
Object.assign(Runtime.Widget.Crud.Rules.Unique.prototype,
{
	...
	onSaveBefore: async function(service)
	{
		...
		var table_name = service.getTableName();
		var q = (new Runtime.ORM.Query()).select().from(table_name).addRawField("count(1) as c");
		/* Add filter */
		if (this.keys)
		{
			for (var i = 0; i < this.keys.count(); i++)
			{
				...
				var value = service.data.get(field_name);
		...
		{
			var filter = provider.getPrimaryFilter(service.getTableName(), service.item.toMap(), true);
